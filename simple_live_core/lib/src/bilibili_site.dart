...
class BiliBiliSite implements LiveSite {
  ...
  Future<LiveRankResult> getLiveRanks(
    ...
    try {
      // 通过房间信息获取主播 uid（ruid）
      var roomInfo = await getRoomInfo(roomId: detail.roomId);
      var ruid = roomInfo["room_info"]?["uid"]?.toString() ??
          roomInfo["anchor_info"]?["base_info"]?["uid"]?.toString() ??
          "";
      if (ruid.isEmpty) {
        return LiveRankResult(title: "在线高能榜", items: []);
      }
      ...
      var list = result["data"]?["OnlineRankItem"] as List? ?? [];
      List<LiveRankItem> items = [];
      for (var i = 0; i < list.length; i++) {
        var item = list[i];
        items.add(
          LiveRankItem(
            rank: asT<int?>(item["userRank"]) ?? (i + 1),
            userName: item["name"]?.toString() ?? "",
            avatar: item["face"]?.toString() ?? "",
            score: item["score"]?.toString() ?? "",
            level: asT<int?>(item["guard_level"]) ?? 0,
          ),
        );
      }
      return LiveRankResult(title: "高能用户", items: items);
    } catch (e) {
      CoreLog.error(e);
      return LiveRankResult(title: "高能用户", items: []);
    }
  }
}
