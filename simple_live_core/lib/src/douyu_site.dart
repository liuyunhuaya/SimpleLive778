...
class DouyuSite implements LiveSite {
  ...
  Future<LiveRankResult> getLiveRanks(
      {required LiveRoomDetail detail}) async {
    final referer = 'https://www.douyu.com/${detail.roomId}';
    final ua =
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36';

    // 优先使用 PR#887 提供的亲密榜接口（intimacyRank）
    try {
      final result = await HttpClient.instance.getJson(
        "https://www.douyu.com/japi/interact/comm/fanshome/rank/top10",
        queryParameters: {"rid": detail.roomId},
        header: {
          'referer': referer,
          'user-agent': ua,
        },
      ...
      final raw = (data is Map) ? data["intimacyRank"] : null;
      final list = (raw is List) ? raw : const [];
      if (list.isNotEmpty) {
        final items = <LiveRankItem>[];
        for (var i = 0; i < list.length; i++) {
          final item = list[i];
          if (item is! Map) continue;
          items.add(LiveRankItem(
            rank: int.tryParse(item["rank"]?.toString() ?? "") ?? (i + 1),
            userName: item["nickname"]?.toString() ?? "",
            avatar: item["avatar"]?.toString() ?? "",
            score: item["value"]?.toString() ?? "0",
            scoreDetail: "亲密度",
          ));
        }
        if (items.isNotEmpty) {
          return LiveRankResult(title: "亲密榜", items: items);
        }
      }
    } catch (e) {
      CoreLog.error(e);
    ...
    try {
      final result = await HttpClient.instance.getJson(
        "https://www.douyu.com/japi/rank/list/topRank",
        queryParameters: {"roomId": detail.roomId},
        header: {
          'referer': referer,
          'user-agent': ua,
        },
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        items.add(LiveRankItem(
          ...
          userName: item["nickName"]?.toString() ??
              ...
              "",
          avatar:
              item["avatar"]?.toString() ?? item["icon"]?.toString() ?? "",
          score: item["score"]?.toString() ??
              item["dyf"]?.toString() ??
              item["totalScore"]?.toString() ??
              "",
          level: int.tryParse(item["level"]?.toString() ?? "0") ?? 0,
        ));
      }
      if (items.isNotEmpty) {
        return LiveRankResult(title: "贡献榜", items: items);
      }
    } catch (e) {
      CoreLog.error(e);
    }
    ...
    // 再兜底：周榜
    try {
      final weekResp = await HttpClient.instance.getJson(
        "https://www.douyu.com/wgapi/livenc/liveweb/anchorrank/getAnchorWeekRank",
        queryParameters: {"rid": detail.roomId},
        header: {
          'referer': referer,
          'user-agent': ua,
      ...
      final dataWeek = (weekResp is Map) ? weekResp["data"] : null;
      final rawWeek = (dataWeek is Map) ? dataWeek["list"] : null;
      final weekList = (rawWeek is List) ? rawWeek : const [];
      final items = <LiveRankItem>[];
      for (var i = 0; i < weekList.length; i++) {
        final item = weekList[i];
        if (item is! Map) continue;
        items.add(LiveRankItem(
          rank: i + 1,
          userName: item["nickName"]?.toString() ??
      ...
      return LiveRankResult(title: "周榜", items: items);
    } catch (e) {
      CoreLog.error(e);
    }

    return LiveRankResult(title: "亲密榜", items: []);
  }
}
